...
if __name__ == '__main__':
    ...
    gu.mount_module_blocks(pwchange,path="/pwchange",privileged_users='!@guest')
    import app
    app.app.queue(max_size=20,default_concurrency_limit=4)
    gu.mount_module_blocks(app,path="",privileged_users='!@guest')
    gu.run(
        root_path = app_schema.get('ROOTPATH',''),
        host = app_schema.get('HOST','127.0.0.1'),
        port = app_schema.get('GRADIO_PORT',8761),
